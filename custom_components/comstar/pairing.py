"""AO mTLS pairing — Comstar AoMtlsService parity."""
    ...
    def inspect(self) -> dict[str, Any]:
        ...
        if paired:
            try:
                ...
                out["client_name"] = material.subject
            except Exception as exc:  # noqa: BLE001
                out["ok"] = False
                out["last_error"] = str(exc)
        return out

    async def enroll(self, enroll_token: str, *, client_name: str | None = None) -> dict[str, Any]:
        try:
            await ReachMtlsEnroller().enroll(
                base_url=self.engine_url,
                enroll_token=enroll_token,
                material_dir=str(self.material_dir),
                common_name=client_name or "comstar-ha",
